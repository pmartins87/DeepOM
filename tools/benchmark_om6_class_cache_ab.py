...
def run_variant(
    *,
    ...
    solver = DenseExternalSamplingCFR(
        ...
        precompute_class_indices=precompute_class_indices,
    )
...
def main() -> None:
    ...
    uncached, uncached_seconds = run_variant(
        class_index=index,
        seed=args.seed,
    ...
    uncached_dps = total_deals / uncached_seconds
    ...
    result = {
        ...
        "uncached": {
            "precompute_showdown_ranks": True,
            "precompute_class_indices": False,
            "train_seconds": uncached_seconds,
            "deals_per_second": uncached_dps,
            "visited_infosets": uncached.visited_infosets(),
            "arrays_sha256": uncached.manifest()["arrays_sha256"],
        },
        "cached": {
            "precompute_showdown_ranks": True,
            "precompute_class_indices": True,
        ...
        "exact_trajectory_match": (
            uncached.manifest()["arrays_sha256"] == cached.manifest()["arrays_sha256"]
        ),
        "speedup_cached_over_uncached": cached_dps / uncached_dps,
    ...
    if not result["exact_trajectory_match"]:
        raise SystemExit("class-index cache changed the CFR trajectory")
    print("OM6_CLASS_CACHE_AB=PASS")
